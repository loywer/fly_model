...
#Матрица перехода из СкСК в ССК
    ...
    #Вычисление Cy
        ...
        cy = cy_1+(c(2Modul_V))cy_2w_vxod[2] + cy_3                   
    #Вычисление Cz
        ...
        cz = cz_1self.betta_vxod+(b(2Modul_V))(cz_2w_vxod[0]+cz_3w_vxod[1]) + cz_4self.DeltaRud_vxod
    ...
    def Aero_Forces(self)
        ...
        Fx=(-1)Aero_Forces_coefficient[0]  self.ro  (Modul_V2)  (Sw  2)
        Fy=Aero_Forces_coefficient[1]  self.ro  (Modul_V2)  (Sw  2)
        Fz=Aero_Forces_coefficient[2]  self.ro  (Modul_V2)  (Sw  2)
    ...
    #Вычисление mx
        ...
        mx = 0.1mx_1  self.betta_vxod + (b(2Modul_V))(mx_2self.w_vxod[0]+mx_3self.w_vxod[1])+0.075mx_4self.DeltaRud_vxod+mx_5    
    #Вычисление my
        ...
        my = my_1self.betta_vxod+(b(2Modul_V))(my_2self.w_vxod[0]+my_3self.w_vxod[1])+0.075my_4self.DeltaRud_vxod + my_5 
    #Вычисление mz
        ...
        mz = mz_1+(c(2Modul_V))2mz_2self.w_vxod[2]+mz_3  
    ...
    def Aero_Moment (self)
        ...
        Mx = Aero_Moment_coefficient[0]  self.ro  b (Modul_V2)  (Sw  2)
        My =Aero_Moment_coefficient[1]  self.ro  b (Modul_V2)  (Sw  2)
        Mz = Aero_Moment_coefficient[2]  self.ro  c (Modul_V2)  (Sw  2)
...
#Интегратор
    def Integrator (self)  
        ...
        angl_new = self.angl_vxod + right_side_of_the_DU_anglself.dt
        koordinat_new = self.koordinat_vxod + right_side_of_the_DU_koordself.dt
...
#Заполняем вектора
            alpha_array.append(self.alpha_vxod)
            betta_array.append(self.betta_vxod)
            V_array.append(self.V_vxod)
            w_array.append(self.w_vxod)
            angl_array.append(self.angl_vxod)
            koordinat_array.append(self.koordinat_vxod)
            t_array.append(t)

#Вызываем функции
            self.n = self.overload()
            self.alpha_vxod,self.betta_vxod = self.new_alpha_betta()
            self.V_a_vxod,self.w_a_vxod = self.finding_accelerations()
            self.V_vxod,self.w_vxod,self.angl_vxod,self.koordinat_vxod = self.Integrator()

            t = t + dt

        return self.n,self.alpha_vxod,self.betta_vxod,self.V_a_vxod,self.w_a_vxod,self.V_vxod,self.w_vxod,self.angl_vxod,self.koordinat_vxod
